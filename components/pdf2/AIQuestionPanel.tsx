...
import { Send, X, Sparkles, GripVertical, Loader2 } from 'lucide-react'
...
export default function AIQuestionPanel({ button, isOpen, buttonScreenPosition, customAnswerDemo, onClose, pdfId }: AIQuestionPanelProps) {
  ...
  // 패널이 열릴 때마다 차트 로딩 state 초기화 + 최소 로딩 시간 보장
  useEffect(() => {
    if (isOpen && button.chartImage) {
      // 로딩 state 초기화
  ...
  }, [isOpen, button.id, button.chartImage])
  ...
  useEffect(() => {
    ...
    console.log('🧮 Calculated position:', { x, y, buttonScreenPosition })
  ...
  return (
    <div
      ...
      className="fixed w-80 bg-white/95 backdrop-blur-sm border border-primary/20 rounded-lg shadow-lg p-3 overflow-y-auto max-h-[600px]"
      style={{
    ...
    >
      ...
      {button.chartImage && (
        <div className="mb-3">
          {(isChartLoading || minLoadingTime) && (
            <div className="p-8 border-2 border-dashed border-primary/20 rounded-lg bg-primary/5">
              <div className="flex flex-col items-center gap-4">
                ...
                <div className="text-center space-y-1">
                  <p className="font-semibold text-slate-900">차트 로딩 중...</p>
          ...
          <img
            src={button.chartImage}
            alt="차트 이미지"
            onLoad={() => setIsChartLoading(false)}
            onError={() => setIsChartLoading(false)}
            className={(isChartLoading || minLoadingTime) ? 'hidden' : 'w-full h-auto rounded-md border border-slate-200'}
          />
        </div>
      )}
